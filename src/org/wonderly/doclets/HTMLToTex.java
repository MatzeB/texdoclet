...
public class HTMLToTex {
	...
	/** keeps track of contexts inside the html formatting
	 * Saves the apropriate string to end the respective context.
	 * (i.e. in an <p> context the string </p> would be saved) 
	 */
	...
	private void leave(String exitString) {
		...
		if (exitString.equals("")) {
			...
			if (i < 0) {
				System.err.println("Warning: No matching opening tag for '" + exitString + "'");
		...
		while (contexts.size() > i) {
			...
			if (i < contexts.size()) {
				System.err.println("Warning: Missing closing tag '" + tag + "'");
				System.err.println("Context: " + str);
			}
			ret.append(tex);
		}
	...
	private boolean match(String needle) {
		...
		pos += needle.length() - 1; /* hacky: the -1 compensates the ++i in the for loop in convert... */
		return true;
	}	
	...
	private String convertToTex(String input) {
		...
		for (pos = 0 ; pos < str.length(); ++pos) {
			...
			case '}':
				...
				break;
			case '{':
				if( str.length() > pos+5 && str.substring(pos,pos+6).equalsIgnoreCase("{@link") ) {
					block = "@link";
					collectBlock = true;
					pos += 5;
				} else {
			...
			case '<':
				if(match("<pre>")) {
					...
					leave("</center>");
				} else if( str.length() > pos+4 && str.substring(pos,pos+5).equalsIgnoreCase("<meta") ){
					...
					ret.append("}");
				} else if( str.length() > pos+4 && str.substring(pos,pos+5).equalsIgnoreCase("<form") ){
					Properties p = new Properties();
					int idx = HTMLToTex.getTagAttrs( str, p, pos+3 );
					pos = idx;
				} else if(match("</form>")) {
					/* nothing */
				} else if( str.length() > pos+5 && str.substring(pos,pos+6).equalsIgnoreCase("<input") ){
					Properties p = new Properties();
					int idx = HTMLToTex.getTagAttrs( str, p, pos+3 );
					pos = idx;
				} else if(match("</input>")) {
					/* nothing */
				} else if( str.length() > pos+4 && str.substring(pos,pos+5).equalsIgnoreCase("<body") ){
					Properties p = new Properties();
					int idx = HTMLToTex.getTagAttrs( str, p, pos+3 );
					pos = idx;
				} else if(match("</body>")) {
					...
					enter("</p>", "\\end{texdocp}");
				} else if( str.length() > pos+2 && str.substring(pos,pos+3).equalsIgnoreCase("<hr") ){
					Properties p = new Properties();
					int idx = HTMLToTex.getTagAttrs( str, p, pos+3 );
					...
					/* nothing */
				} else if( str.length() > pos+4 && str.substring(pos,pos+4).equalsIgnoreCase("<img") ){
					Properties p = new Properties();
				...
				} else if( str.length() > pos+2 && str.substring(pos,pos+2).equalsIgnoreCase("<a") ){
					Properties p = new Properties();
					int idx = HTMLToTex.getTagAttrs( str, p, pos+3 );
				...
				} else if( str.length() > pos+3 && str.substring(pos,pos+3).equalsIgnoreCase("<ol") ){
					Properties p = new Properties();
					int idx = HTMLToTex.getTagAttrs( str, p, pos+3 );
					pos = idx;
					...
					itemcnt = 0;
				} else if( str.length() > pos+2 && str.substring(pos,pos+3).equalsIgnoreCase("<dl") ){
					Properties p = new Properties();
					int idx = HTMLToTex.getTagAttrs( str, p, pos+3 );
					pos = idx;
					...
					if( itemcnts.isEmpty() == false )
						itemcnt = itemcnts.pop().intValue();
				} else if( str.length() > pos+3 && str.substring(pos,pos+3).equalsIgnoreCase("<ul") ){
					Properties p = new Properties();
					int idx = HTMLToTex.getTagAttrs( str, p, pos+3 );
					pos = idx;
					ret.append( "\\begin{itemize}" );
					...
					tblinfo.endRow(ret);
				} else if(str.length() > pos+5 && str.substring(pos,pos+6).equalsIgnoreCase("<table") ){
					Properties p = new Properties();
					int idx = getTagAttrs( str, p, pos+6 );
					pos = idx;
					stackTable( p, ret, str, pos);
				} else if(str.length() > pos+2 && str.substring(pos,pos+3).equalsIgnoreCase("<tr") ){
					Properties p = new Properties();
					int idx = getTagAttrs( str, p, pos+3 );
					pos = idx;
					tblinfo.startRow(ret,p);
				} else if(str.length() > pos+2 && str.substring(pos,pos+3).equalsIgnoreCase("<td") ){
					Properties p = new Properties();
					int idx = getTagAttrs( str, p, pos+3 );
					pos = idx;
					tblinfo.startCol(ret, p);
				} else if(str.length() > pos+2 && str.substring(pos,pos+3).equalsIgnoreCase("<th") ){
					Properties p = new Properties();
					int idx = getTagAttrs( str, p, pos+3 );
					pos = idx;
					tblinfo.startHeadCol(ret,p);
				} else if( str.length() > pos+4 && str.substring(pos,pos+5).equalsIgnoreCase("<font") ){
					Properties p = new Properties();
			...
			case '/':
				...
				break;
			case '&':
				if( str.length() > pos+4 && str.substring(pos,pos+2).equals("&#") ) {
				...
				} else if( str.length() > pos+4 && str.substring(pos,pos+5).equalsIgnoreCase("&amp;") ) {
					ret.append("\\&");
					pos+=4;
				} else if( str.length() > pos+5 && str.substring(pos,pos+6).equalsIgnoreCase("&nbsp;") ) {
					ret.append("\\phantom{ }");
					pos+=5;
				} else if( str.length() > pos+3 && str.substring(pos,pos+4).equalsIgnoreCase("&lt;") ) {
					ret.append("\\textless ");
					pos += 3;
				} else if( str.length() > pos+3 && str.substring(pos,pos+4).equalsIgnoreCase("&gt;") ) {
